...
async function main() {
  ...
  console.log(
    '⚙️ Threshold:',
    (await contract.similarityThreshold()).toString() + '%'
  );
  ...
  const testContent =
    'This is a test post about blockchain technology and smart contracts.';
  ...
  const checkStatus = async () => {
    try {
      ...
      console.log(
        `📊 Status: ${statusNames[post.status]} | Score: ${post.similarityScore}% | CID: ${post.ipfsCID || 'None'}`
      );
      ...
      if (post.status === 2) {
        ...
        console.log(
          '🌐 View at: https://gateway.pinata.cloud/ipfs/' + post.ipfsCID
        );
        return true;
