...
const ORACLE_SERVICE_URL =
  process.env.REACT_APP_ORACLE_SERVICE_URL || 'http://localhost:3001';
  ...
  async submitPost(content, username) {
    try {
      ...
      if (postSubmittedEvent) {
        ...
        await this.sendContentToOracle(
          postId,
          content,
          username,
          walletAddress
        ...
        if (post.ipfsCID) {
          try {
            ...
            const ipfsResponse = await fetch(
              `https://gateway.pinata.cloud/ipfs/${post.ipfsCID}`
            );
            ...
            console.error(
              `Failed to fetch IPFS content for post ${post.id}:`,
              ipfsError
            );
          }
        }
